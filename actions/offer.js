...
const fetchOffers = (accessToken = {}) => {
	return (dispatch) => {
		...
		.then((offers) => {
			...
			dispatch(
				receiveOffers(offers, Date.now())
			);

		}).catch((error) => {

			dispatch(
				failOffersRequest(error)
			);
...
const fetchOffer = (offer = {}, accessToken = 0) => {
	return (dispatch) => {
		...
		.then((refreshedOffer) => {
			...
			dispatch(
				receiveOffer(refreshedOffer, Date.now())
			);

		}).catch((error) => {

			dispatch(
				failOfferRequest(error)
			);
...
const debouncedPut = debounce((dispatch, offer = {}, accessToken = {}) => {
  ...
  dispatch(
	...
	.then((updatedOffer) => {
		...
		dispatch(
			receiveOffer(updatedOffer, Date.now())
		);
    ...
    dispatch(
      clearValidationErrors('offer')
		...
		.then((savedOffer) => {
			...
			dispatch(
				receiveOffer(savedOffer, Date.now())
			);
...
export const deleteOffer = (offer = {}, accessToken = {}) => {
	return (dispatch) => {
		...
		.then((response) => {
			...
			dispatch(
				deletedOffer(offer, response.success)
			);
