...
const fetchOfferRequests = (accessToken = '') => {
	return (dispatch) => {
		...
		dispatch(
			requestOfferRequests(accessToken)
		);
		...
		.then((offerRequests) => {
			...
			dispatch(
				receiveOfferRequests(offerRequests, Date.now())
			);

		}).catch((error) => {

			dispatch(
				failOfferRequestsRequest(error)
			);
...
const fetchOfferRequest = (offerRequest = {}, accessToken = 0) => {
	return (dispatch) => {
		...
		dispatch(
			requestOfferRequest(offerRequest)
		);
		...
		.then((refreshedOfferRequest) => {
			...
			dispatch(
				receiveOfferRequest(refreshedOfferRequest, Date.now())
			);

		}).catch((error) => {

			dispatch(
				failOfferRequestRequest(error)
			);
...
const shouldFetchOfferRequest = (state = {}, offerRequest = {}) => {
	...
	if(offerRequest.isFetching){
		return false;
	}else if(!offerRequest || !offerRequest.lastUpdated || offerRequest.lastUpdated === 0){
		return true;
	}else{
...
const debouncedPut = debounce((dispatch, offerRequest = {}, accessToken = '') => {
  ...
  dispatch(
	...
	return fetchApi('offer-request/' + offerRequest.id, 'PUT', {offerRequest}, accessToken)
	.then((updatedOfferRequest) => {

		dispatch(
			receiveOfferRequest(updatedOfferRequest, Date.now())
		);
		...
		dispatch(
			failOfferRequestPut(error, offerRequest)
		);
...
export const postOfferRequest = (offerRequest = {}, accessToken = '') => {
    ...
    dispatch(
      clearValidationErrors('offerRequest')
		...
		dispatch(
			postOfferRequest_(offerRequest)
		);
		...
		.then((savedOfferRequest) => {
			...
			dispatch(
				receiveOfferRequest(savedOfferRequest, Date.now())
			);
			...
			dispatch(
				failOfferRequestPost(error, offerRequest)
			);
...
export const deleteOfferRequest = (offerRequest = {}, accessToken = '') => {
	return (dispatch) => {
		...
		dispatch(
			deleteOfferRequest_(offerRequest)
		);

		return fetchApi('offer-request/' + offerRequest.id, 'DELETE', {}, accessToken)
		.then((response) => {

			dispatch(
				deletedOfferRequest(offerRequest, response.success)
			);

			if(!response.success){
				failOfferRequestDelete('The API couldn\'t delete the offerRequest!', offerRequest)
			}
			...
			dispatch(
				failOfferRequestDelete(error, offerRequest)
			);
